...
class qtr {
  /**
   * Make it TRUE to output debug info on '/tmp/qtr.log'.
   */
  const DEBUG = FALSE;
  ...
  public static function __callStatic($function, $args) {
    $qtr_function = self::function_name($function);
    if (!function_exists($qtr_function)) {
      self::log_var($qtr_function, 'Autoload');
      // Try to load the file that contains the function.
      if (!self::load_search_dirs($function) or !function_exists($qtr_function)) {
        $dir = dirname(self::file($fname));
        $dir = str_replace(DRUPAL_ROOT, '', $dir);
        self::log_var("Raising an exception.\n", 'Not found');
        throw new Exception("Function $qtr_function could not be found on $dir");
      }
    }
    return call_user_func_array($qtr_function, $args);
  }

...
function project_import($origin, $project, $lng, $path, $uid = NULL) {
  qtr::messages("Import project: $origin/$project/$lng: $path");

  // Check that the project exists.
  $pguid = qtr::db_query(
    'SELECT pguid FROM {qtr_projects} WHERE pguid = :pguid',
...
function _process_po_file($origin, $project, $tplname, $lng, $file, $filename, $uid = 1) {
  qtr::messages("Import PO file: $filename");

  // Get the template id.
  $potid = qtr::db_query(
    'SELECT potid FROM {qtr_templates}
...
function _add_file($file, $filename, $potid, $lng, $headers, $comments, $uid = 1) {
  ...
  $row = qtr::db_query(
    'SELECT potid, lng FROM {qtr_files} WHERE hash = :hash',
    array(':hash' => $hash)
  )->fetchAssoc();
    ...
    if ($row['potid']==$potid and $row['lng']==$lng) {
      ...
      $row1 = qtr::db_query(
        'SELECT p.origin, p.project, t.tplname
         ...
         LEFT JOIN {qtr_projects} p ON (t.pguid = p.pguid)
         WHERE t.potid = :potid',
...
function _get_string_sguid($entry) {
  ...
  $get_sguid = "SELECT sguid FROM {qtr_strings} WHERE sguid = '$sguid'";
  if ($sguid == qtr::db_query($get_sguid)->fetchField()) {
    return $sguid;
  }
  else {
    return NULL;
...
function _add_translation($sguid, $lng, $translation, $uid = 1) {
  ...
  $get_tguid = 'SELECT tguid FROM {qtr_translations} WHERE tguid = :tguid';
  $args = array(':tguid' => $tguid);
  if (qtr::db_query($get_tguid, $args)->fetchField())  return $tguid;
...
function _make_snapshots($origin, $project, $lng, $path, $uid) {
  ...
  qtr::project_snapshot_save($origin, $project, $lng, $snapshot_file, $uid);
  ...
  qtr::project_snapshot($origin, $project, $lng, $diff_comment, $export_mode = 'original', $uid);
  ...
  qtr::project_snapshot($origin, $project, $lng, $diff_comment, $export_mode = 'most_voted', $uid);
}

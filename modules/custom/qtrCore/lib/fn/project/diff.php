  ...
  $export_mode = 'most_voted', $preferred_voters = NULL, $uid = NULL)
{
  ...
  qtr::project_export($origin, $project, $lng, $export_dir,
  ...
  qtr::project_snapshot_get($origin, $project, $lng, $snapshot_file);
...
function project_diff_add($origin, $project, $lng, $file_diff, $file_ediff, $comment = NULL, $uid = NULL)
{
  ...
  $max_nr = qtr::db_query(
    'SELECT MAX(nr) AS max_nr FROM {qtr_diffs}
...
function project_diff_list($origin, $project, $lng) {
  $diff_list = qtr::db_query(
    'SELECT nr, time, comment FROM {qtr_diffs}
     WHERE pguid = :pguid AND lng = :lng
     ORDER BY time ASC',
    array(
      ':pguid' => sha1($origin . $project),
...
function project_diff_get($origin, $project, $lng, $nr, $format = 'diff') {
  ...
  $diff = qtr::db_query(
    "SELECT $diff_field FROM {qtr_diffs}

...
function project_add($origin, $project, $path, $uid = NULL) {
  qtr::messages("Adding project: $origin/$project: $path");

  // Erase the project if it exists.
  qtr::project_del($origin, $project, $erase = TRUE, $purge = FALSE);
...
function _process_pot_file($pguid, $tplname, $file, $filename, $uid) {
  qtr::messages("Importing POT file: $filename");

  // Create a new template.
  $potid = qtr::db_insert('qtr_templates')
    ->fields(array(
        'tplname' => $tplname,
        'filename' => $filename,
        'pguid' => $pguid,
...
function _add_string($entry, $uid) {
  ...
  $count = qtr::db_update('qtr_strings')

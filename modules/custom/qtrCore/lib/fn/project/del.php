...
function project_del($origin = NULL, $project = NULL, $erase = TRUE, $purge = TRUE) {
  ...
  $get_pguid = qtr::db_select('qtr_projects', 'p')->fields('p', array('pguid'));
  ...
  $potid_list = qtr::db_query(
    'SELECT potid FROM {qtr_templates} t
  ...
  if ($erase) {
    foreach ($pguid_list as $pguid) {
      qtr::db_delete('qtr_diffs')->condition('pguid', $pguid)->execute();
      qtr::db_delete('qtr_snapshots')->condition('pguid', $pguid)->execute();
    }
  }
...
function _delete_template($potid) {
  ...
  qtr::db_query(
    'UPDATE {qtr_strings} AS s
     INNER JOIN (SELECT sguid FROM {qtr_locations} WHERE potid = :potid) AS l
             ON (l.sguid = s.sguid)
     SET s.count = s.count - 1',
    array(':potid' => $potid));

  // Delete the locations, files, and the template itself.
  qtr::db_delete('qtr_locations')->condition('potid', $potid)->execute();
  qtr::db_delete('qtr_files')->condition('potid', $potid)->execute();
  qtr::db_delete('qtr_templates')->condition('potid', $potid)->execute();
}
...
function _unsubscribe_users($pguid_list) {
  $query = 'SELECT origin, project FROM {qtr_projects} WHERE pguid IN (:pguid_list)';
  $args = [':pguid_list' => $pguid_list];
  $result = qtr::db_query($query, $args)->fetchAll();

...
module_load_include('php', 'qtrCore', 'lib/gettext/POWriter');
...
{
  qtr::messages("Export project: $origin/$project/$lng: $path");
  ...
  if ($export_mode == 'preferred' and empty($preferred_voters)) {
    $account = user_load(qtr::user_check($uid));
  ...
  $result = qtr::db_query(
    'SELECT t.tplname, f.filename
     FROM {qtr_files} f
     LEFT JOIN {qtr_templates} t ON (f.potid = t.potid)
  ...
  foreach ($result as $row) {
    qtr::messages("Export PO file: $origin/$project/$lng: $row->filename");
...
function _export_po_file($potid, $lng, $filename, $export_mode = 'most_voted', $preferred_voters = NULL) {
  ...
  $row = qtr::db_query(
    'SELECT headers, comments FROM {qtr_files}
...
function _get_original_translations($potid, $lng)
{
  ...
  $file_content = qtr::db_query(
    'SELECT content FROM {qtr_files}
     WHERE potid = :potid AND lng = :lng',
    [':potid' => $potid, ':lng' => $lng]
  )->fetchField();

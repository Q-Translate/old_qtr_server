  ...
  $export_mode = 'most_voted', $preferred_voters = NULL, $uid = NULL)
{
  ...
  if (filesize($file_diff) != 0 or filesize($file_ediff) != 0) {
    qtr::project_diff_add($origin, $project, $lng, $file_diff, $file_ediff, $comment);
    qtr::project_snapshot_save($origin, $project, $lng, $export_file, $uid);
  }
  else {
    qtr::messages(t('Diffs are empty, no snapshot saved.'));
  }
...
function project_snapshot_get($origin, $project, $lng, $file) {
  ...
  $snapshot = qtr::db_query(
    'SELECT snapshot FROM {qtr_snapshots}
  ...
  if ($snapshot) {
    ...
    qtr::project_export($origin, $project, $lng, $tmpdir, $export_mode='original');
...
function project_snapshot_save($origin, $project, $lng, $file, $uid = NULL) {
  ...
  qtr::db_delete('qtr_snapshots')
    ->condition('pguid', sha1($origin . $project))
    ->condition('lng', $lng)
    ->execute();

  // Save the new snapshot.
  qtr::db_insert('qtr_snapshots')
    ->fields(array(
        'pguid' => sha1($origin . $project),
        'lng' => $lng,

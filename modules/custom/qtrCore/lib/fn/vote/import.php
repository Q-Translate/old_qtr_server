...
function _add_translation($sguid, $lng, $translation, $uid) {
  ...
  $query = 'SELECT * FROM {qtr_translations} WHERE tguid = :tguid';
  $args = array(':tguid' => $tguid);
  $result = qtr::db_query($query, $args)->fetch();

  if (!$result) {
    // Add the translation for this string.
    qtr::translation_add($sguid, $lng, $translation, $uid);
  }
  else {

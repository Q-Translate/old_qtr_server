...
function report_statistics($lng = NULL, $origin = NULL, $project = NULL) {
  ...
  if ($origin == NULL) {
    $qtr_translations = 'qtr_translations';
    $qtr_votes = 'qtr_votes';
  }
  else {
    include_once __DIR__ . '/_create_tmp_tables.inc';
    _create_tmp_tables($lng, $origin, $project);
    $qtr_translations = 'qtr_tmp_translations';
    $qtr_votes = 'qtr_tmp_votes';
  }
  ...
  $sql_count_translations =
     "SELECT count(*) as cnt FROM {$qtr_translations}
      ...
      AND umail != ''";
  $sql_count_votes =
     "SELECT count(*) as cnt FROM {$qtr_votes}
      WHERE $condition";
  ...
  foreach (array('week', 'month', 'year') as $period) {
    ...
    $nr_votes = qtr::db_query($sql_count_votes, $args)->fetchField();

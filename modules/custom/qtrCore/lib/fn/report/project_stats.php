...
function report_project_stats($origin, $project, $lng) {
  ...
  $sql = "
    SELECT COUNT(*)
    FROM {qtr_projects} p
    INNER JOIN {qtr_templates} tpl ON (tpl.pguid = p.pguid)
    INNER JOIN {qtr_locations} l ON (l.potid = tpl.potid)
    WHERE p.origin = :origin AND p.project = :project
  ";
  $args = [':origin' => $origin, ':project' => $project];
  $stats['strings'] = qtr::db_query($sql, $args)->fetchField();
  ...
  $sql = "
    ...
    INNER JOIN {qtr_templates} tpl ON (tpl.pguid = p.pguid)
    INNER JOIN {qtr_locations} l ON (l.potid = tpl.potid)
    INNER JOIN {qtr_strings} s ON (s.sguid = l.sguid)
    INNER JOIN {qtr_translations} t ON (t.sguid = s.sguid AND t.lng = :lng)
    WHERE p.origin = :origin AND p.project = :project
  ";
  $args[':lng'] = $lng;
  $stats['translated'] = qtr::db_query($sql, $args)->fetchField();
  ...
  $sql = "
    ...
    WHERE p.origin = :origin AND p.project = :project
      AND t.tguid is NULL
  ";
  $stats['untranslated'] = qtr::db_query($sql, $args)->fetchField();
  ...
  $sql = "
    ...
    WHERE p.origin = :origin AND p.project = :project
  ";
  $stats['translations'] = qtr::db_query($sql, $args)->fetchField();
  ...
  $sql = "
    ...
    WHERE p.origin = :origin AND p.project = :project
  ";
  $stats['votes'] = qtr::db_query($sql, $args)->fetchField();
  ...
  $sql = "
    ...
    WHERE p.origin = :origin AND p.project = :project
  ";
  $stats['contributors'] = qtr::db_query($sql, $args)->fetchField();

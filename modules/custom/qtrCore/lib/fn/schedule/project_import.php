...
function schedule_project_import($origin, $project, $uploaded_file) {
  ...
  $query = 'SELECT uid FROM {qtr_projects} WHERE pguid = :pguid';
  $args = [':pguid' => sha1($origin . $project)];
  $uid = qtr::db_query($query, $args)->fetchField();
  ...
  if (!drupal_move_uploaded_file($uploaded_file['tmp_name'], $file_uri)) {
    qtr::messages(t('Failed to move uploaded file.'), 'error');
    return;
  }
  ...
  try {
    ...
    qtr::messages($e->getMessage(), 'error');
    return;
  }

  // Schedule the import.
  qtr::queue('import_project', [[
